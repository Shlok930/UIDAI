...
import { UserCheck, AlertTriangle, AlertCircle, Info, Zap, Brain } from "lucide-react";
...
const COLORS = ["#f59e0b","#f0a830","#eb9850","#d4836a","#be6e85","#a859a0","#9244ba","#7c2fd5","#6619ef","#5004ff"];

const Tip = ({ active, payload, label }: any) => {
  if (!active || !payload?.length) return null;
  return (
    <div className="glass rounded-xl p-3 border border-brand-amber/20 text-xs min-w-[180px]">
      <p className="text-brand-amber font-bold mb-2">{label}</p>
      {payload.map((p: any) => (
...
export default function DemographicAnalysis({ stateRanking, trends, anomalies, forecast }: Props) {
  ...
  return (
    <div className="space-y-10">
      <SectionHeader tag="Demographic Intelligence" title="Demographic Update Deep Analysis" subtitle="State ranking, monthly trends, anomaly detection and 3-month forecast for demographic updates (5–17 & 17+)" icon={UserCheck} color="green" />

      {/* ── Trends ── */}
      <div className="grid lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          className="lg:col-span-2 glass rounded-2xl p-5 border border-brand-green/10">
          <p className="text-xs text-slate-400 mb-4 uppercase tracking-widest">Monthly Demographic Updates — Youth vs Adult</p>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trends.monthly} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <defs>
                  <linearGradient id="demoAmberGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#f59e0b" stopOpacity={0.25} />
                    <stop offset="95%" stopColor="#f59e0b" stopOpacity={0} />
                  </linearGradient>
                  <linearGradient id="demoGreenGrad" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area type="monotone" dataKey="total" stroke="#f59e0b" strokeWidth={2.5} fill="url(#demoAmberGrad)" name="Total" dot={false} activeDot={{ r: 5, fill: "#f59e0b" }} />
                <Area type="monotone" dataKey="youth" stroke="#10b981" strokeWidth={2} fill="url(#demoGreenGrad)" name="Youth (5–17)" dot={false} activeDot={{ r: 4, fill: "#10b981" }} />
                <Area type="monotone" dataKey="adult" stroke="#3b82f6" strokeWidth={2} fill="none" name="Adult (17+)" dot={false} activeDot={{ r: 4, fill: "#3b82f6" }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
          ...
          {trends.insights.map((ins, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.12 }}
              className="rounded-xl bg-brand-green/5 border border-brand-green/15 p-3 text-xs text-slate-300 leading-relaxed">
              <span className="text-brand-green font-bold mr-1">›</span> {ins}
            </motion.div>
          ))}
          {trends.monthly.length > 0 && (() => {
            const last = trends.monthly[trends.monthly.length - 1];
            const tot = last.total || 1;
            const adultPct = Math.round(last.adult / tot * 100);
            return (
              <div className="mt-auto rounded-xl bg-white/[0.02] border border-white/[0.06] p-3">
                ...
                <div className="space-y-2">
                  {[["Adult (17+)", adultPct, "#3b82f6"], ["Youth (5–17)", 100-adultPct, "#10b981"]].map(([label, pct, col]) => (
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          className="glass rounded-2xl p-5 border border-brand-green/10">
          <p className="text-xs text-slate-400 mb-4 uppercase tracking-widest">Top 10 States — Demographic Updates</p>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={top10} layout="vertical" margin={{ top: 0, right: 20, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(16,185,129,0.05)" horizontal={false} />
                ...
                <motion.div key={s.state} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-xs font-mono text-brand-amber ml-2 shrink-0"><AnimatedCounter end={s.total} /></span>
                    </div>
      ...
      {/* ── Anomalies ── */}
      <div>
        <p className="text-xs text-slate-400 uppercase tracking-widest mb-4">Demographic Anomaly Detection</p>
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div className={`rounded-lg ${cfg.bg} p-2`}><p className="text-slate-400">Youth</p><p className={`font-bold ${cfg.color}`}>{a.youth.toLocaleString("en-IN")}</p></div>
                </div>
                <p className="mt-3 text-[10px] text-slate-400 leading-relaxed">
                  <Zap size={9} className="inline mr-1 text-brand-amber" />
                  {a.youth > a.adult ? `Youth demographic updates spiked to ${a.youth.toLocaleString("en-IN")} — atypical for this region.` : `Adult demographic updates unusually high at ${a.adult.toLocaleString("en-IN")} — isolation forest outlier.`}
                </p>
              </motion.div>
            );
          })}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          ...
          <div className="flex items-center gap-2 mb-4">
            <span className="text-xs text-slate-400 uppercase tracking-widest">Demographic 3-Month Forecast</span>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartForecast} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <defs>
                  <linearGradient id="demoAmberBar" x1="0" y1="0" x2="0" y2="1">
                ...
                <Tooltip content={<Tip />} />
                <Legend wrapperStyle={{ fontSize: 11, color: "#94a3b8" }} />
                <Bar dataKey="total" name="Total Updates" fill="url(#demoAmberBar)" radius={[6,6,0,0]} maxBarSize={40} />
                <Line type="monotone" dataKey="youth" name="Youth" stroke="#10b981" strokeWidth={2.5} strokeDasharray="6 3" dot={{ fill: "#10b981", r: 5 }} />
                <Line type="monotone" dataKey="adult" name="Adult" stroke="#3b82f6" strokeWidth={2.5} strokeDasharray="6 3" dot={{ fill: "#3b82f6", r: 5 }} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        <div className="flex flex-col gap-4">
          {chartForecast.map((d, i) => (
            <motion.div key={d.period} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.1 }}
              className="glass rounded-2xl p-4 border border-brand-green/15 relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-brand-green/5 to-transparent pointer-events-none" />
              <div className="relative">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-bold text-brand-green uppercase tracking-widest">{d.period}</span>
                ...
                <div className="flex gap-4 mt-1">
                  ...
                  <div><p className="text-[10px] text-slate-500">Adult</p><p className="text-sm font-bold text-brand-blue"><AnimatedCounter end={d.adult} duration={1500} /></p></div>
                </div>
                <div className="mt-3 h-1 rounded-full bg-white/5">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${d.confidence}%` }} transition={{ duration: 1, delay: 0.5 + i * 0.15 }}
                    className="h-1 rounded-full bg-gradient-to-r from-brand-green to-brand-blue" />
                </div>
              </div>
            </motion.div>
          ))}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
            className="rounded-xl bg-brand-green/5 border border-brand-green/20 p-3 text-xs text-slate-400">
            <Brain size={12} className="inline mr-1.5 text-brand-green" />
            Linear regression on 9 months of demographic data. Youth & adult modelled separately.
          </motion.div>
        </div>
      </div>
    </div>
